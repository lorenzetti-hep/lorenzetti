...
void EventTape::push_back( IAlgorithm *alg )
{
  m_algs.push_back(alg);
}


StatusCode EventTape::initialize()
{
  MSG_INFO( "Initialize generator..." );
...
StatusCode EventTape::finalize()
{
  MSG_INFO( "Finalize the Event generator." );
  
  for( auto &alg : m_algs ){
    if ( alg->finalize().isFailure() ){
      MSG_ERROR( "It's not possible to finalize the event filter with name " << alg->name() );
    }
  }
