...
cmd( f,  'source /setup_here.sh %s'%args.volume  )
...
command = "prun_job.py -o {OUTPUT} -c '{COMMAND}' -mt {NTHREADS} -n {NJOBS}".format( VOLUME=volume,
                                                                                     COMMAND=command,
                                                                                     NTHREADS=ncpu,
                                                                                     NJOBS=args.njobs,
                                                                                     OUTPUT=output)
...
command = reco_script + ' -i {INPUT} -o {OUTPUT} -nt {NTHREADS} --ntuple {NTUPLE}'.format( INPUT     = input,
                                                                                           OUTPUT    = output,
                                                                                           NTHREADS  = ncpu,
                                                                                           NTUPLE    = args.ntuple)

if args.enableMagneticField:
  command += ' --enableMagneticField'
