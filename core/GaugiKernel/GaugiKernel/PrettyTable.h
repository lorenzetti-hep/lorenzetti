 ...
 */
enum class PrettyTableColumnFormat
{
  AUTO,
  SCIENTIFIC,
  FIXED,
 ...
 * PrettyTable<std::string, double, int, std::string> vt("Name", "Weight", "Age", "Brother");
 *
...
public:
   ...
   */
  PrettyTable(std::vector<std::string> headers,
   ...
   */
  void setColumnFormat(const std::vector<PrettyTableColumnFormat> & column_format)
  {
...
protected:
  ...
  void print_each(TupleType && t, StreamType & stream, std::integral_constant<size_t, I>)
  {
    ...
    if (!_column_format.empty())
    {
      ...
      if (_column_format[I] == PrettyTableColumnFormat::SCIENTIFIC)
        stream << std::scientific;

      if (_column_format[I] == PrettyTableColumnFormat::FIXED)
        stream << std::fixed;

      if (_column_format[I] == PrettyTableColumnFormat::PERCENT)
  ...
  /// Column Format
  std::vector<PrettyTableColumnFormat> _column_format;
