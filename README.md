...
The output file generated by the previusly command will be used as input in this `reco_trf` job option. All events generated will be propagated throut of the generic calorimeter and the step points information will be collected by the reconstruction steps to build the low-level calorimeter information (cells), emulate the eletronic pulse, simulate the Pileup, estimate the cells's energy and build the RoI (Region Of Interesting). To apply the reconstruction step:
