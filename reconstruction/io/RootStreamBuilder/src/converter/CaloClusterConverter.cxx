...
bool CaloClusterConverter::serialize( std::string &key, SG::EventContext &ctx, TTree *tree ) const
{

  std::vector<xAOD::CaloCluster_t> *clusters_t=nullptr;
  std::string branch_name = "xAOD__CaloCluster";
  
  if ( tree->FindBranch(branch_name.c_str())){
    tree->SetBranchStatus((branch_name).c_str(), 1);
    tree->SetBranchAddress( (branch_name).c_str() , &clusters_t     );
    tree->GetBranch(branch_name.c_str() );
  }else{
    tree->Branch( (branch_name).c_str(), &clusters_t     );
  }
  ...
  SG::ReadHandle<xAOD::CaloCellContainer> cells( key, ctx );
  ...
  for (const auto event : **container.ptr() ){
    xAOD::CaloCluster_t event_t;
    convert( event, event_t );
    clusters_t->push_back(event_t);
  }
  tree->Fill();
  return true;
}
...
bool CaloClusterConverter::deserialize( std::string &key , int &evt, TTree* tree, SG::EventContext &ctx) const
{
  std::vector<xAOD::CaloCluster_t> *clusters_t=nullptr;
  std::string branch_name = "xAOD__CaloCluster";
  tree->SetBranchAddress(  branch_name.c_str() , &clusters_t );
  ...
  SG::WriteHandle<xAOD::CaloClusterContainer> container(key, ctx);
  container.record( std::unique_ptr<xAOD::CaloClusterContainer>(new xAOD::CaloClusterContainer()));

  for( auto& event_t : *clusters_t)
  {
    xAOD::CaloCluster  *event=nullptr;
