...
RootStreamNTUPLEMaker::RootStreamNTUPLEMaker(std::string name) : IMsgService(name),
                                                                 Algorithm()
{
...
StatusCode RootStreamNTUPLEMaker::bookHistograms(EventContext &ctx) const
{
  ...
  outputTree->Branch("event_number", &eventNumber);
...
StatusCode RootStreamNTUPLEMaker::deserialize(int evt, EventContext &ctx) const
{
  ...
  InitBranch(outputTree, "event_number", &eventNumber);
  ...
  for (auto &electron_t : *collection_electrons)
  {
    ...
    outputTree->Fill(); // each cluster should be 1 entry on ntuple file
    cluster_link++;
  }
...
bool RootStreamNTUPLEMaker::computeForwardDecision(xAOD::CaloCluster *cluster, std::string workingpoint) const
{
  ...
  {
    if (cluster->secondLambda() > m_secondLambdaCuts[0])
      return false;
    if (cluster->lateralMom() > m_lateralMomCuts[0])
      return false;
    if (cluster->longitudinalMom() > m_longMomCuts[0])
      return false;
    if (cluster->fracMax() < m_fracMaxCuts[0])
      return false;
    if (cluster->secondR() > m_secondRCuts[0])
      return false;
    if (cluster->lambdaCenter() > m_lambdaCenterCuts[0])
      return false;
    return true;
  }
  ...
  {
    if (cluster->secondLambda() > m_secondLambdaCuts[1])
      return false;
    if (cluster->lateralMom() > m_lateralMomCuts[1])
      return false;
    if (cluster->longitudinalMom() > m_longMomCuts[1])
      return false;
    if (cluster->fracMax() < m_fracMaxCuts[1])
      return false;
    if (cluster->secondR() > m_secondRCuts[1])
      return false;
    if (cluster->lambdaCenter() > m_lambdaCenterCuts[1])
      return false;
    return true;
  }
  ...
  {
    if (cluster->secondLambda() > m_secondLambdaCuts[2])
      return false;
    if (cluster->lateralMom() > m_lateralMomCuts[2])
      return false;
    if (cluster->longitudinalMom() > m_longMomCuts[2])
      return false;
    if (cluster->fracMax() < m_fracMaxCuts[2])
      return false;
    if (cluster->secondR() > m_secondRCuts[2])
      return false;
    if (cluster->lambdaCenter() > m_lambdaCenterCuts[2])
      return false;
    return true;
  }
  return false;
